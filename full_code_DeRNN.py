...
def get_batch():
    ...
    for i in range(0, 4):
        ex_factor.append(timeseries[i][:])

    for i in range(4, len(timeseries)):
        seq.append(timeseries[i][:])
    ...
    plt.plot(xs[0][:], seq[0][BATCH_START: BATCH_START + TIME_STEPS], 'r')
    plt.plot(xs[0][:], res[0][BATCH_START: BATCH_START + TIME_STEPS], 'b--')
    plt.plot(xs[0][:], ex_factor[0][BATCH_START: BATCH_START + TIME_STEPS], 'g')
    plt.show()
